...
class MergeReadinessTests(unittest.TestCase):
    ...
    def test_mixed_hazard_destructive_marked_pending_clears_the_hold(self) -> None:
        # admin#1495 R2 finding 3813789228: additive=applied plus
        # destructive=pending is the SAFE mid-sequence state (additive-apply
        # → compatible deploy → destructive-apply), but the undifferentiated
        # map held it forever. Directioned rows make it representable: the
        # pending destructive-marked row must NOT hold readiness.
        text = _mutate(
            self._with_blocks(),
            '  deploy_order: "pending"',
    ...
    def test_mixed_hazard_plain_pending_row_still_holds(self) -> None:
        # Legacy safe default: an UNMARKED pending row always holds — only
        # the explicit destructive marker releases post-deploy work.
        text = _mutate(
            self._with_blocks(),
            '  deploy_order: "pending"',
            '  deploy_order: "hazard_documented"\n  hazard_direction: "mixed"',
        ...
        text = _mutate(
            text,
            "  applied_state: {}",
            '  applied_state:\n    prod:\n      "0043_drop_column": "pending"',
        )
        result = evaluate_state_text(text)
        self.assertEqual(result["errors"], [])
        self.assertTrue(monitor_extract(text)["merge_readiness_hold"])

    def test_directioned_row_outside_mixed_hazard_is_rejected(self) -> None:
        # Directioned rows are a mixed-hazard vocabulary — under an additive
        # hazard the destructive marker would silently release a hold the
        ...
        self.assertTrue(
            any(
                "directioned rows require hazard_direction 'mixed'" in error
                for error in result["errors"]
            ),
            result["errors"],
        )
    ...
    def test_malformed_directioned_row_holds(self) -> None:
        # A directioned row with no status is unproven state: the validator
        # rejects it and the defensive derivation keeps the hold live.
        text = _mutate(
            self._with_blocks(),
            '  deploy_order: "pending"',
            '  deploy_order: "hazard_documented"\n  hazard_direction: "mixed"',
...
class OperationContractDifferentialTests(unittest.TestCase):
    ...
    def test_failed_descendant_after_failed_predecessor_is_rejected(self) -> None:
        # algo#1216 R2 finding 3813491655: the guard rejected complete /
        # pending / retryable after a failed predecessor but accepted a
        # FAILED descendant — yet `failed` proves an ATTEMPT, and the
        # ordered write-ahead executor stops (or skips forward as
        # skipped_dependency) at the first failure, so it can never attempt
        # the next operation at all. A failed-after-failed ledger conceals a
        # mutation fired after its safety prerequisite failed.
        schema_errors = validate_operation_collection(
            ["probe.parent", "probe.child"],
            {"probe.parent": "failed", "probe.child": "failed"},
            label="probe",
        )
        self.assertTrue(
            any(
                "'probe.child' is failed after failed/skipped predecessor" in e
                for e in schema_errors
            ),
            schema_errors,
        )
        # Pass-through side: skipped_dependency IS what the executor writes
        # past a failure — the legitimate ledger must stay accepted.
        self.assertEqual(
            validate_operation_collection(
                ["probe.parent", "probe.child"],
                {"probe.parent": "failed", "probe.child": "skipped_dependency"},
                label="probe",
            ),
            [],
        )
