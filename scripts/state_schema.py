...
def validate_operation_collection(
    ...
    for operation_id in operations:
        ...
        if operation_id in in_flight or not has_result:
            saw_unfinished = True
    # algo#1216 finding 3792942228, extended by finding 3813491655:
    # dependency semantics existed only in the planner — the schema and
    # runner accepted a failed parent with a completed dependent. The
    # ordered-prefix protocol above already implies the edge DIRECTION
    # (each operation depends on its predecessors in the planned order), so
    # the portable check needs no persisted edge list: ANY attempted result
    # (complete/pending/retryable — and `failed`, which equally proves an
    # attempt) AFTER a failed/skipped predecessor in the same plan cannot
    # have been produced by the write-ahead executor, which stops (or skips
    # forward as skipped_dependency) at the first failure. Only
    # skipped_dependency and absent results are legal past that point.
            ...
            if not isinstance(applied, dict):
                ...
                for env, migrations in applied.items():
                    ...
                    for migration, status in migrations.items():
                        row_path = f"{env_path}.{_safe_key(str(migration))}"
                        if isinstance(status, dict):
                            # admin#1495 R2 finding 3813789228: a mixed
                            # hazard's safe sequence is additive-apply →
                            # compatible deploy → destructive-apply, so its
                            # destructive rows are POST-merge work by
                            # contract. Directioned rows exist exactly to
                            # say which is which; they are meaningful only
                            # under a mixed hazard (purely destructive
                            # never holds, purely additive has no
                            # destructive rows).
                            for key in status:
                                if key not in ("status", "direction"):
                                    self.error(
                                        f"{row_path}: unknown key"
                                        f" {_safe_key(str(key))!r}"
                                    )
                            self.check_enum(
                                status.get("status"),
                            ...
                            self.check_enum(
                                ...
                                f"{row_path}.direction",
                            )
                            if value.get("hazard_direction") != "mixed":
                                self.error(
                                    f"{row_path}: directioned rows require"
                                    " hazard_direction 'mixed' — use the"
                                    " plain status string otherwise"
                                )
                        else:
                            self.check_enum(
                                status,
...
def _applied_row_holds(row: Any, hazard_direction: Any) -> bool:
    """admin#1495 R2 finding 3813789228: under a MIXED hazard the contract
    orders destructive work AFTER the compatible deploy, so a pending row
    explicitly marked destructive is post-merge work and must not hold
    merge readiness — holding on it made the safe additive → deploy →
    destructive sequence unrepresentable. Legacy plain-string rows keep
    holding on "pending" (safe default), and a malformed directioned row
    holds (unproven state never releases a hold)."""

    if isinstance(row, dict):
        status = row.get("status")
        if not isinstance(status, str) or status not in APPLIED_STATE_ENUM:
            return True
        if (
            hazard_direction == "mixed"
            and row.get("direction") == "destructive"
        ):
            return False
...
def monitor_extract(text: str) -> dict[str, Any]:
    ...
    if isinstance(constants, dict):
        # Runner-facing (algo#1216 R2 finding 3813491642): only int-valued
        # entries travel — the runner enforces the resolved work cap from
        # the LAUNCH-time canonical; malformed entries are dropped, never a
        # crash, and the default lives runner-side.
        extract["monitor_constants"] = {
            key: value
    ...
    if isinstance(handoffs, dict):
        for kind_name, record in handoffs.items():
            status = record.get("status") if isinstance(record, dict) else None
            statuses.append(status if isinstance(status, str) else "malformed")
            # algo#1216 R2 finding 3813491661: the flat list is kind-blind,
            # so the runner could not ask "is the QA handoff still idle?" —
            # expose the kind-keyed view alongside it.
            if isinstance(kind_name, str):
                extract["handoff_kind_statuses"][kind_name] = (
                    status if isinstance(status, str) else "malformed"
                )
            ...
            if not isinstance(applied, dict) or not applied:
                ...
                for env_map in applied.values():
                    if not isinstance(env_map, dict) or not env_map:
                        ...
                        _applied_row_holds(row, gate.get("hazard_direction"))
                        for row in env_map.values()
                    ):
                        hold = True
