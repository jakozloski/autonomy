...
- Claude Fable 5.1 (`claude-fable-5-1`) — the reviewer leg: an optional supplemental review voice (starts `high`, `max` permitted) plus the mandatory seats — fallbacks, escalation, Phase 6 monitor-session ownership — at `max`
...
Fable 5 is the base: `max` for substantive implementation, architecture, and difficult debugging; `ultracode` — or `max` with delegated agents — when the work splits into independently implementable components. GPT-6 Astra is the standing reviewer: `ultra` for broad PR reviews, with the lead validating every finding and checking cross-component interactions, and `max` for focused, difficult reviews and investigating specific findings. Routine steps may run `high` (either vendor) or `xhigh` (Codex). Claude Fable 5.1 adds an optional supplemental review pass — starting at `high`, the conservative one-rung reading of [CodeRabbit's published pipeline results](https://www.coderabbit.ai/blog/fable-5-1-model-review), where low beat high and medium/max went untested, so cheaper starts are proven viable and `max` stays permitted — and staffs the mandatory seats (fallbacks, escalation, monitor ownership) at `max`. Every finding, from any voice, counts only with a concrete failing scenario or demonstrated code path. When a discussion stalls or reviewers disagree, the escalation voice is one not already in that discussion rather than a rerun of one that already spoke.
