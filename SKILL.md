...
- Keeper repositories run a pre-human review step (user directive, 2026-08-11): implementer → R2 (`r2-keeper` bot) → human. The monitor's R2 review gate (monitor-exit-handoffs.md) defers reviewer requests, assignee transfer, and the QA handoff until R2 approves the PR; the PR stays assigned to the invoking user through R2 rounds, and no push may land while an R2 ask is pending (a push supersedes the running review, which can take ~80 minutes). The FIRST R2 ask on a PR is never automated — the workflow blocks and asks the user; after R2's first review, fix-and-re-ask rounds run automatically until R2 approves. The first human review permanently disarms the gate: post-human fix rounds hand straight back to that human reviewer, never through R2.
