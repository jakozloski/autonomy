...
These values override defaults in delegated skills and adapters. Match compute shape to task shape, on both vendors, across two tiers. **`max`** — the deepest non-delegating reasoning tier (`high → xhigh → max` on Codex) — is the focused tier: difficult questions, focused debugging, one plan verdict, one disputed finding. **Breadth** is `ultra` reasoning on GPT (maximum reasoning **plus automatic delegation to parallel subagents**) and the `ultracode` workflow mode on Claude: select it for large code reviews, broad research, and building features across multiple components. Choose from the task actually in front of the voice, default to `max` when the shape is ambiguous, and record every breadth-tier selection in state and the Decision Audit Trail. Claude review voices under the read-only boundary cannot delegate (Agent/Task are denied), so a large review on the Claude side gains breadth from the loop's multiple passes, not from a delegating session. The floors below are depth floors, and the gate proves the Codex selection supports both `max` and `ultra` up front.
...
| Claude reviewer | Fable 5.1 at `max`   | The always-runs structured review, every Claude review fallback                                                                   | DEGRADE    |
...
The reviewer is a distinct leg, not a distinct lineage: both Claude legs run the fable/mythos family at `max` — never Opus or any lower tier — every review voice runs in a fresh read-only context that never judges its own output, and Codex remains the standing cross-vendor verdict, so the session that writes the code is never the one that approves it. The base does the work; the two reviewers judge it — the mandatory Phase 2 verdict is always Codex's, while Phase 4 Codex challenges are tiered (Medium and up; Small and skill-only passes are Claude-only by design). Escalation is staffed by a voice not already in that discussion — the Claude reviewer joins a stalled plan review, and a fresh read-only base-lineage context adjudicates when the two reviewers cannot converge on a diff.
...
- Use Claude Fable 5 (`claude-fable-5`, CLI alias `fable`) at `max` effort for the working side: explorers, delegated work (Phase 6 dispatched workers included), and every Claude voice that is not a review voice — the one carve-out is the Phase 6 monitor session itself, owned per the Monitor orchestrator role above. `max` is the focused tier — the right shape for one hard problem: a difficult question, focused debugging, one failing path. For breadth-shaped working tasks — broad research sweeps, building a feature across multiple components — use the `ultracode` workflow mode (max reasoning plus automatic parallel delegation), and record the tier choice in the Decision Audit Trail.
...
- Use Claude Fable 5.1 (`claude-fable-5-1`, CLI alias `fable`) at `max` effort for every Claude review voice: the always-runs structured review, every Claude review fallback, and the reviewer's escalation seat in a stalled plan review. It sits **next to** the mandatory Phase 2 Codex verdict, never in place of it (Phase 4 Codex participation is tiered), and it is read-only on both execution paths under exactly the same tool boundary as every review voice. Never staff a Claude review voice with Opus or any model below the Fable 5.1 floor.
...
3. Never silently downgrade below the GPT-6 Astra, base Fable 5, or reviewer Fable 5.1 floors, at each voice's policy effort tier. Newer-model auto-selection is upward only, stays within the fable/mythos lineage for both Claude legs (and the GPT catalog for Codex), and is always recorded in state and the audit trail; `ultra` and `ultracode` are the breadth tier — selected for large code reviews, broad research, and multi-component feature builds, recorded like every tier choice, never a substitute for `max` depth on one hard problem.
