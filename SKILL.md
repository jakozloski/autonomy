...
3. Never silently downgrade below the GPT-5.6 Sol/max, base Fable 5/max, or reviewer Fable 5.1/max floors. Newer-model auto-selection is upward only, stays within the fable/mythos lineage for both Claude legs (and the GPT catalog for Codex), and is always recorded in state and the audit trail; `ultra` and `ultracode` are breadth modes for genuinely decomposable tasks, never a deeper setting for one hard problem.
