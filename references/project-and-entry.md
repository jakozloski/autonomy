...
- **Bind the classification (admin#1495 r16 F7):** persist `gstack_integration.classification_fingerprint` — SHA-256 (64 lowercase hex) over the inputs the selectors were derived from: the merge-base SHA, the head SHA, and a digest of the worktree diff (empty-tree digest for the issue/context fallback). The selectors are only trustworthy relative to the tree they classified, so this is what makes selector freshness checkable: recompute the fingerprint with `change_type` after Phase 3, and treat a mismatch as a stale classification to redo, never to patch by hand. The state validator requires the field (with the other core selectors) on every full-tier state.
