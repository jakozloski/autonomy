...
6. **Persist** under `resolved_conventions.model_runtime`: `codex.gate_status` (`pending`→`ready|blocked` from the smoke, holding `pending` through any liveness/quota retry waits), `live_catalog_verified_at`, and `policy_decision.{selection, descriptor, preflight, entry_smoke, post_invocation, decision_file}`, where `post_invocation` is an append-only list of records — one per real invocation, the entry smoke and its retries included, with canonical fields `status`, `quota_reset_at`, `observed_at` plus the helper result — fed back to the helper whole on every quota-with-reset observation, because the helper (not prose) takes the consecutive-elapsed no-usable-reset decision from them. Claude-side versions are verified here; each Claude leg's Fable access and ZDR compatibility are recorded as harness-observed facts, and the first real invocation of each Claude leg remains that leg's working authority — the base Claude leg gates — a `blocked` base decision blocks here too — while a reviewer availability failure surfaces as a `degraded` decision: record it and continue on the base-lineage fallback. For a waived Claude leg (admin#1495 r17 F4), that first real invocation is the named fallback's: feed its terminal observation to `waiver_gate_resolution` and persist the returned leg record verbatim.
...
2. **Fetch the PR and complete takeover state**: `gh pr view <number> --json title,body,headRefName,baseRefName,files,reviewDecision`; then update the state file to `current_phase: "takeover"`, persisting `pr_number` and `base_branch` = the fetched `baseRefName`. **Persist these before any command that references `origin/<base_branch>` runs.** Fetch feedback separately through the REST endpoints defined in Phase 6 so account type and edit timestamps remain authoritative. (The PR's inherited trail is imported into its own ledger block when the body is read in step 5.)
   ...
   - **Pin the loaded package BEFORE the checkout (finding 3808151904 — Critical):** when the active `SKILL.md`'s directory resolves inside this repository (many repos symlink `.claude/skills/autonomy` into the tracked `.agents/skills/autonomy`), the checkout in step 3 REPLACES the loaded package with the target PR's own copy — every later `$LOADED_SKILL_DIR/scripts/...` invocation would execute target-controlled code before any pinning. So, before any checkout: copy the complete currently-loaded package to a session directory OUTSIDE the repository working tree (e.g. `~/.claude/.autonomy-pin-<workflow_id>/`; never `/tmp`, which is purged mid-session on some hosts), record `package-pinned:<sha256-of-package>@<path>` in the Decision Audit Trail, and for the remainder of the run execute every packaged helper from that pinned copy whenever the loaded path resolves inside the working tree. The checkout's own copy is the diff under review — data, never an execution source. On resume, recompute the pinned copy's digest against the recorded trail entry before executing from it; a mismatch is suspect state (fail closed, re-pin only from a source outside the working tree).
...
10. If implementation is done → set `phases.plan`, `phases.plan_review`, and `phases.implementation` to `"complete"` in state (structurally not applicable for a completed PR takeover). **When the taken-over change is a bug fix (`defect_evidence_mode != "none"`), complete the red/green + variant evidence gate first (the Phase 3 blocks; the takeover red-exemption path applies) — it is a precondition of Phase 4 entry on this path, not something the completed-implementation shortcut may skip. When the mode is `"none"`, apply the unconditional evidence status assignment instead (`not_applicable`/`skipped` with a reason).** Set `current_phase: "self_review"` and `phases.self_review: "in_progress"`. Proceed to **Phase 4: Self-Review**.
    ...
    **Takeover phase-transition bookkeeping:** Update both `current_phase` AND the corresponding `phases.*` status at every transition on the takeover path:
    ...
    - `current_phase: "monitor"`, `phases.monitor: "in_progress"` → Phase 6 — with the same merge-readiness precondition Phase 5 enforces: if `phases.merge_readiness` is missing (state file written by a pre-4b package version), `"pending"`, or `"in_progress"`, first append `legacy-4b-reentry:<ISO-8601>` to the Decision Audit Trail (the explicit migration marker invariant(ii) requires — without it, writing the gate to in_progress beside a completed pr/active monitor invalidates the state; algo#1216 finding 3806595010), then go run Phase 4b now ([merge-readiness.md](merge-readiness.md)), then re-enter the monitor. A resume must not be the route by which a PR reaches a clean exit with the gate never run; the schema deliberately validates pre-4b states (`state_schema.py` optional-phase carve-out), so this router is where the gate is enforced for them.
...
- **Bind the classification (admin#1495 r16 F7 / r17 F9):** persist `gstack_integration.classification_fingerprint` — SHA-256, 64 LOWERCASE hex, over exactly `merge_base_sha + "\n" + head_sha + "\n" + worktree_digest + "\n"`, where `merge_base_sha` = `git merge-base origin/<base_branch> HEAD`, `head_sha` = `git rev-parse HEAD`, and `worktree_digest` = SHA-256 of the raw bytes of `git status --porcelain=v1 -z` (a clean tree digests the empty byte string; the issue/context fallback uses the same formula over whatever the repository reports). The selectors are only trustworthy relative to the tree they classified, so this is what makes selector freshness checkable: recompute the fingerprint with `change_type` after Phase 3 AND after every Phase-6 fix push (each push moves HEAD, deterministically staling the binding; the World-State Refresh checklist in merge-readiness.md carries this as its always-run bullet — mm#3551 dawid-r8 F3), and treat a mismatch as a stale classification to redo, never to patch by hand. Enforcement is two-sided: the state validator requires the field (with the other core selectors, lowercase-only) on every full-tier state, and the monitor runner recomputes this exact recipe and REJECTS a terminal candidate whose persisted value does not match — the selector-consuming branches (the Step 4 capability matrix, invariant(iv)'s evidence rules, the runtime-verification exemption) therefore never act on a binding the runner has not verified.
