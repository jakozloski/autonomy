...
1. **gstack `/review` adapter** (primary, when gstack available and `change_type != skill_only`):
   ...
   - Every Codex invocation in this adapter runs the policy-selected model (floor GPT-5.6 Sol) at `max` reasoning — `codex exec` via `-m <selected>`, `codex review` via `-c 'model="<selected>"'` (it rejects `-m`), both with `-c 'model_reasoning_effort="max"'` (see Model Configuration); Claude review passes run on the recorded reviewer-leg decision (floor Opus 5, or its recorded fallback) and fresh-base escalation passes on the selected base model (floor Fable 5)
