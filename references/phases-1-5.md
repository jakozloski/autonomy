...
**Tool selection is mandatory with fallback chain:** items 2-4 select the Claude-side review tooling only — when Codex is healthy after a succeeded Phase 2 gate, the standing Astra review at the task-shape tier still runs (direct `codex exec`/`codex review` forms per Model Configuration) regardless of which item the chain lands on; the tier rows' Codex-unavailability substitution is the only path that replaces it.
